...
export function TeacherAssistWeekWorkspaceScreen() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setInstructionalWeekHref(`/teacher-assist/week/${instructionalWeek.id}`);
      } catch {
  ...
  const weekContext = (workspace?.week_context ?? {}) as Record<string, any>;
  const period = (workspace?.period ?? {}) as Record<string, any>;
  ...
  const recommendations = (workspace?.recommendations ?? {}) as Record<string, any>;
  const topReuse = (recommendations.recommended_for_this_week?.top_reusable ?? []) as Array<Record<string, any>>;
      ...
      try {
        if (actionKey === "generate_next_week") {
          ...
          await saveWeekTemplate(periodId, {
            name: `${weekContext.period_title ?? "Week"} Template`,
            artifact_type: "WEEK",
          ...
          const href = `/teacher-assist/week/${created.id}`;
  ...
  return (
    <div id="week-workspace-panel" className="space-y-4">
      <header className="ta-panel p-4">
        ...
        <h1 className="mt-1 text-2xl font-semibold text-slate-900">{weekContext.period_title ?? period.title ?? "Selected week"}</h1>
        <p className="mt-1 text-sm text-slate-600">
          {weekContext.subject_name ?? "Subject"} · Grade {weekContext.grade_display_name ?? weekContext.grade_level ?? "—"} ·{" "}
          {weekContext.pacing_guide_title ?? "Pacing guide"}
        </p>
      ...
      {loading || !workspace ? (
        ...
        <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          <article className="ta-panel space-y-3 p-4">
            ...
            <p className="text-sm text-slate-600">{period.description ?? weekContext.notes ?? "No week description yet."}</p>
            <div className="flex flex-wrap gap-1.5">
              {(period.objectives ?? weekContext.objectives ?? []).map((row: { objective_code?: string }, index: number) => (
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-sm text-slate-700">
                {(period.resources ?? weekContext.resources ?? []).slice(0, 8).map((row: { title?: string; resource_title?: string }, index: number) => (
                  <li key={`${row.title ?? row.resource_title}-${index}`}>{row.resource_title ?? row.title ?? "Resource"}</li>
                ))}
              </ul>
            </div>
            {period.teacher_notes || weekContext.notes ? (
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Teacher notes</p>
                <p className="mt-1 text-sm text-slate-600">{period.teacher_notes ?? weekContext.notes}</p>
              </div>
            ) : null}
          </article>
            ...
            {topReuse.length > 0 ? (
              <div>
                ...
                <div className="mt-2 space-y-1.5">
                  {topReuse.slice(0, 3).map((row) => (
                    <Link
                      key={`${row.entity_type}-${row.entity_id}`}
                      href={String(row.navigation_href ?? "#")}
                    ...
                    >
                      ...
                      <span className="ml-2 text-slate-500">Score {row.reuse_score?.score ?? "—"}</span>
                    </Link>
                  ))}
                </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {row.artifact_type ?? row.entity_type}
                    </p>
                  </div>
                  <Link href={String(row.navigation_href ?? "#")} className="ta-button-secondary text-xs">
                    Open
                  </Link>
                </div>
              ))}
