...
from oziebot_api.services.teacher_assist_v2.package_demo_backfill import (
    backfill_demo_package_for_user,
)
...
def main() -> None:
    ...
    print(
        f"Backfilled package {result['package_id']} ({result['title']}) with {result['artifact_count']} artifacts."
    )
    if result.get("qr_student_packet"):
        packet = result["qr_student_packet"]
        print(
            f"QR student packet generated for {packet['student_count']} students (packet_id={packet['packet_id']})."
        )
