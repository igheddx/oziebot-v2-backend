...
class TeacherAssistLessonReflectionVersion(Base):
    __tablename__ = "teacher_assist_lesson_reflection_versions"
    ...
    lesson_reflection_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("teacher_assist_lesson_reflections.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    lesson_reflection: Mapped["TeacherAssistLessonReflection"] = relationship(
        "TeacherAssistLessonReflection",
        back_populates="versions",
        foreign_keys=[lesson_reflection_id],
    )
