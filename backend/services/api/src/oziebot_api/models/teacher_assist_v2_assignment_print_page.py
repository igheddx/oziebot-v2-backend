...
class TeacherAssistV2AssignmentPrintPage(Base):
    __tablename__ = "teacher_assist_v2_assignment_print_pages"

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    packet_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("teacher_assist_v2_assignment_print_packets.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    packet: Mapped["TeacherAssistV2AssignmentPrintPacket"] = relationship(
