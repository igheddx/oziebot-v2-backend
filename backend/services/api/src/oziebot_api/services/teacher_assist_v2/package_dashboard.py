...
def serialize_package_summary(
    ...
    *,
    ...
    return {
        ...
        "can_close_out": can_close_out_package(
            stored_status=row.status, effective_status=effective
        ),
...
def list_teacher_instructional_packages(
    ...
    *,
    ...
    if school_year_id is not None:
        stmt = stmt.where(
            TeacherAssistV2InstructionalPackage.platform_school_year_id == school_year_id
        )
    ...
    for row in rows:
        ...
        if subject_id is not None and str(subject_id) not in {
            str(value) for value in row.subject_ids_json
        }:
            continue
        summary = serialize_package_summary(
            row, subject_names=names, settings=settings, today=today
        )
...
def get_instructional_package_detail_enriched(
    ...
    *,
    ...
    row = db.scalars(select(TeacherAssistV2InstructionalPackage).where(*_pkg_filter)).one_or_none()
    if row is None:
        raise LookupError("Instructional package not found")

    detail = get_instructional_package_detail(
    ...
    detail.update(
        {
            ...
            "plan_start_date": row.plan_start_date.isoformat(),
            "plan_end_date": row.plan_end_date.isoformat(),
            "primary_pacing_guide_id": str(row.primary_pacing_guide_id)
            if row.primary_pacing_guide_id
            else None,
            "subject_names": _subject_names_for_package(db, row),
            "can_close_out": can_close_out_package(
                stored_status=row.status, effective_status=effective
