...
def _subject_pairs(
    db: Session, pkg: TeacherAssistV2InstructionalPackage
...
def _tomorrow_entry(plan: dict, week_number: int, subject_name: str, today_dow: str) -> dict | None:
...
def _end_of_day(
    *,
    ...
    tomorrow_focuses = [
        {
            "subject": s["subject_name"],
            "focus": (s.get("tomorrow") or {}).get("student_goal")
            or (s.get("tomorrow") or {}).get("teacher_goal"),
            "builds_on_today": (s.get("tomorrow") or {}).get("builds_on_today"),
        }
        for s in subjects_today
        if s.get("tomorrow")
        and (s["tomorrow"].get("student_goal") or s["tomorrow"].get("teacher_goal"))
    ]
...
def build_today_classroom(
    ...
    *,
    ...
    active: list[tuple[TeacherAssistV2InstructionalPackage, int]] = [
        (pkg, wn) for pkg in all_pkgs if (wn := _today_week_number(pkg, today)) is not None
    ]
