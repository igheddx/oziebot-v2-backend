...
def _trend_direction(recent_levels: list[str], prior_levels: list[str]) -> str:
    if not recent_levels:
        return "stable"
    recent_rank = sum(MASTERY_LEVEL_RANK.get(level, 0) for level in recent_levels) / len(
        recent_levels
    )
    if not prior_levels:
        return "stable"
        ...
        *,
        ...
        if objective_id is not None:
            evidence_query = evidence_query.where(
                TeacherAssistInstructionalEvidence.objective_id == objective_id
            )
        if standard_id is not None:
            evidence_query = evidence_query.where(
                TeacherAssistInstructionalEvidence.standard_id == standard_id
            )
        if class_id is not None:
            evidence_query = evidence_query.where(
                TeacherAssistInstructionalEvidence.class_id == class_id
            )
        ...
        evidence_rows = list(
            db.scalars(
                evidence_query.order_by(TeacherAssistInstructionalEvidence.created_at.desc())
            ).all()
        )
        ...
        if standard_id is not None:
            mastery_query = mastery_query.where(
                TeacherAssistMasteryEvaluation.standard_id == standard_id
            )
        if class_id is not None:
            mastery_query = mastery_query.where(
                TeacherAssistMasteryEvaluation.mastery_matrix_id.is_not(None)
            )
        ...
        objective_code = (
            ...
            if objective
            else getattr(standard, "code", None)
        )
        ...
        levels = [
            row.mastery_level for row in confirmed_evidence + mastery_rows if row.mastery_level
        ]
        ...
        return {
            ...
            "mastery_pct": _pct(
                distribution.get("mastery", 0) + distribution.get("advanced", 0), total
            ),
    ...
    def calculate_for_scope(
        ...
        *,
        ...
        if subject_id is not None:
            evidence_query = evidence_query.where(
                TeacherAssistInstructionalEvidence.subject_id == subject_id
            )
        if instructional_week_id is not None:
            evidence_query = evidence_query.where(
                TeacherAssistInstructionalEvidence.instructional_week_id == instructional_week_id
        ...
        return {
            "scope": {
                ...
                "instructional_week_id": str(instructional_week_id)
...
def _students_near_mastery(
    ...
    return [
        {"student_identifier": student, "developing_count": count}
        for student, count in near.items()
    ][:10]
