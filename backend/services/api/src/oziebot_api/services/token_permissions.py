...
class TokenPermissionService:
    ...
    @staticmethod
    def is_token_tradable_for_user(
        ...
        user_permission = db.query(UserTokenPermission).filter(
    ...
    def get_user_tradable_tokens(db: Session, user_id: UUID) -> list[dict]:
        ...
        """
        result = db.query(PlatformTokenAllowlist).join(
            UserTokenPermission,
            and_(
    ...
    def enable_token_for_user(
        ...
        if not platform_token:
            raise ValueError(
                f"Platform token {platform_token_id} not found or is disabled"
            )

        # Check if permission already exists
        existing = db.query(UserTokenPermission).filter(
            UserTokenPermission.user_id == user_id,
            UserTokenPermission.platform_token_id == platform_token_id,
        ).first()
    ...
    def disable_token_for_user(
        ...
        """
        permission = db.query(UserTokenPermission).filter(
            UserTokenPermission.user_id == user_id,
            UserTokenPermission.platform_token_id == platform_token_id,
        ).first()
    ...
    def get_user_token_permission(
        ...
        return db.query(UserTokenPermission).filter(
            UserTokenPermission.user_id == user_id,
            UserTokenPermission.platform_token_id == platform_token_id,
        ).first()

    @staticmethod
    def list_user_token_permissions(
    ...
    def initialize_user_tokens(
        ...
        """
        enabled_tokens = db.query(PlatformTokenAllowlist).filter(
            PlatformTokenAllowlist.is_enabled == True,  # noqa: E712
        ).all()
