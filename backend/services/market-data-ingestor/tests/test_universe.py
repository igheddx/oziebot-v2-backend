...
def test_universe_filters_platform_and_user_enabled_tokens():
    ...
    with eng.begin() as conn:
        ...
        conn.execute(text("CREATE TABLE user_token_permissions (user_id TEXT, platform_token_id TEXT, is_enabled BOOLEAN NOT NULL)"))

        conn.execute(text("INSERT INTO users (id, is_active) VALUES ('u1', 1), ('u2', 0)"))
        conn.execute(
            text(
