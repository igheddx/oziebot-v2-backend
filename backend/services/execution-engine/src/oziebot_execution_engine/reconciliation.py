...
class ReconciliationService:
    def __init__(self, settings, execution_service: ExecutionService, coinbase_client: CoinbaseExecutionClient, *, credential_loader=None) -> None:
        ...
        self._credential_loader = credential_loader or execution_service.load_live_credentials
    ...
    def reconcile_all_live(self) -> list[ReconciliationSummary]:
        ...
        with self._engine.begin() as conn:
            tenant_ids = [str(row.tenant_id) for row in conn.execute(stmt).mappings().all()]
        results: list[ReconciliationSummary] = []
        for tenant_id in tenant_ids:
            self._touch()
            results.append(self.reconcile_tenant(uuid.UUID(tenant_id), TradingMode.LIVE))
            self._touch()
        return results

    def reconcile_tenant(self, tenant_id: uuid.UUID, trading_mode: TradingMode) -> ReconciliationSummary:
        if trading_mode != TradingMode.LIVE:
            ...
            return ReconciliationSummary(str(tenant_id), trading_mode, 0, 0, 0, 0, 0, skipped=True)
        ...
        try:
            ...
            self._touch()
            balances = self._coinbase.list_balances(api_key_name=api_key_name, private_key_pem=private_key_pem)
            self._touch()
            open_orders = self._coinbase.list_open_orders(api_key_name=api_key_name, private_key_pem=private_key_pem)
            self._touch()
            fills = self._coinbase.list_fills(api_key_name=api_key_name, private_key_pem=private_key_pem)
            self._touch()
            ...
            return ReconciliationSummary(str(tenant_id), TradingMode.LIVE, 0, 0, 0, 0, 0)

        open_by_id = {str(item.get("order_id") or item.get("id")): item for item in open_orders}
        ...
        for order in orders:
            if order["trading_mode"] != TradingMode.LIVE.value:
                continue
            repaired, fill_count = self._reconcile_order(order, open_by_id, fills_by_order.get(str(order.get("venue_order_id") or ""), []))
        ...
        self._touch()
        return ReconciliationSummary(str(tenant_id), TradingMode.LIVE, len(orders), repaired_orders, repaired_fills, repaired_positions, balance_drifts)

    def _reconcile_order(self, order: dict[str, Any], open_by_id: dict[str, dict[str, Any]], external_fills: list[dict[str, Any]]) -> tuple[bool, int]:
        ...
        if new_fills:
            ...
            submission = ExecutionSubmission(
                ...
                fills=new_fills,
                raw_payload={"reconciliation": True, "order": external_order, "fills": external_fills},
            )
            self._execution._persist_fills_and_positions(str(order["id"]), request, submission)
            repaired = True
        ...
        if external_order is not None:
            external_status = str(external_order.get("status") or external_order.get("order_status") or "OPEN").lower()
            if external_status in {"open", "pending"} and current_state == ExecutionOrderStatus.SUBMITTED:
            ...
            if repaired_fill_count > 0:
                target_state = ExecutionOrderStatus.FILLED
            elif current_state in {ExecutionOrderStatus.SUBMITTED, ExecutionOrderStatus.PENDING, ExecutionOrderStatus.PARTIALLY_FILLED}:
                target_state = ExecutionOrderStatus.CANCELLED
        ...
        if repaired:
            self._audit(
                ...
                internal_snapshot={"before_state": internal_state.value, "after_state": target_state.value},
    ...
    def _repair_terminal_or_state(self, request: ExecutionRequest, order: dict[str, Any], target_state: ExecutionOrderStatus) -> None:
        order_id = str(order["id"])
        ...
        if target_state == ExecutionOrderStatus.CANCELLED:
            ...
            current_cost = self._current_buy_cost_cents(order_id) if request.side == Side.BUY else 0
            ...
            self._execution._set_order_reserved_locked(order_id, reserved_cash_cents=0, locked_cash_cents=max(0, locked - releasable_locked))
            self._execution._set_order_state(order_id, ExecutionOrderStatus.CANCELLED, cancelled_at=_utcnow())
            return
        if target_state == ExecutionOrderStatus.FILLED:
            current_cost = self._current_buy_cost_cents(order_id) if request.side == Side.BUY else 0
            locked = int(order.get("locked_cash_cents") or 0)
            if request.side == Side.BUY and locked > current_cost:
                self._execution._settle_capital(request, locked - current_cost, 0, order_id)
                self._execution._set_order_reserved_locked(order_id, reserved_cash_cents=0, locked_cash_cents=current_cost)
            self._execution._set_order_state(order_id, ExecutionOrderStatus.FILLED, completed_at=_utcnow())
            return
        self._execution._set_order_state(order_id, target_state)

    def _missing_external_fills(self, order_id: str, external_fills: list[dict[str, Any]]) -> list[ExecutionFill]:
        known: set[str] = set()
        with self._engine.begin() as conn:
            rows = conn.execute(text("SELECT venue_fill_id FROM execution_fills WHERE order_id = :order_id"), {"order_id": order_id}).all()
            known = {str(row[0]) for row in rows}
        out: list[ExecutionFill] = []
        for row in external_fills:
            fill_id = str(row.get("trade_id") or row.get("entry_id") or row.get("fill_id") or "")
            ...
            out.append(
                ExecutionFill(
                    ...
                    occurred_at=datetime.fromisoformat(str(row.get("trade_time") or row.get("created_time") or _utcnow().isoformat()).replace("Z", "+00:00")),
                    raw_payload=row,
                )
            )
        return out
    ...
    def _request_from_order(self, order: dict[str, Any]) -> ExecutionRequest:
        ...
        return ExecutionRequest(
            ...
            price_hint=_to_decimal(order["avg_fill_price"] or Decimal(str(order.get("requested_notional_cents") or 0)) / max(_to_decimal(order["quantity"]), Decimal("1")), "0"),
    ...
    def _rebuild_positions(self, tenant_id: uuid.UUID) -> int:
        with self._engine.begin() as conn:
            ...
            existing = conn.execute(
                text("SELECT * FROM execution_positions WHERE tenant_id = :tenant_id AND trading_mode = 'live'"),
                {"tenant_id": str(tenant_id)},
            ).mappings().all()

        grouped: dict[tuple[str, str, str, str], dict[str, Decimal | int]] = {}
        for trade in trades:
            key = (str(trade["user_id"]), str(trade["strategy_id"]), str(trade["symbol"]), str(trade["trading_mode"]))
            item = grouped.setdefault(key, {"quantity": Decimal("0"), "avg_entry_price": Decimal("0"), "realized_pnl_cents": 0})
            ...
            if str(trade["side"]) == Side.BUY.value:
                ...
                total_cost = (item["quantity"] * item["avg_entry_price"]) + (qty * price)  # type: ignore[operator]
                item["quantity"] = new_qty
                item["avg_entry_price"] = total_cost / new_qty if new_qty > 0 else Decimal("0")
            else:
                item["quantity"] = max(Decimal("0"), item["quantity"] - qty)  # type: ignore[operator]
                if item["quantity"] == 0:
                    item["avg_entry_price"] = Decimal("0")
            item["realized_pnl_cents"] = int(item["realized_pnl_cents"]) + int(trade["realized_pnl_cents"])

        repaired = 0
        now = _utcnow()
        existing_map = {
            (str(row["user_id"]), str(row["strategy_id"]), str(row["symbol"]), str(row["trading_mode"])): row
            for row in existing
        }
        with self._engine.begin() as conn:
                ...
                if row is None:
                    conn.execute(
                        ...
                        ),
                        {
                            ...
                            "avg_entry_price": str(Decimal(str(rebuilt["avg_entry_price"])).quantize(Decimal("0.00000001"))),
                    ...
                    repaired += 1
                    continue
                if str(row["quantity"]) != str(rebuilt["quantity"]) or str(row["avg_entry_price"]) != str(Decimal(str(rebuilt["avg_entry_price"])).quantize(Decimal("0.00000001"))) or int(row["realized_pnl_cents"] or 0) != int(rebuilt["realized_pnl_cents"]):
                    conn.execute(
                        text(
    ...
    def _reconcile_balances(self, tenant_id: uuid.UUID, balances: list[dict[str, Any]]) -> int:
        usd_total_cents = 0
        for row in balances:
            currency = str((row.get("available_balance") or {}).get("currency") or row.get("currency") or "")
            if currency != "USD":
                continue
            available = _to_decimal((row.get("available_balance") or {}).get("value") or row.get("available") or "0")
            hold = _to_decimal((row.get("hold") or {}).get("value") or row.get("hold_value") or "0")
            usd_total_cents += int(((available + hold) * Decimal("100")).quantize(Decimal("1")))
        ...
        if abs(internal_total - usd_total_cents) <= self._settings.reconciliation_balance_drift_tolerance_cents:
            self._audit(
                tenant_id=tenant_id,
                trading_mode=TradingMode.LIVE,
                scope="balances",
    ...
    def _current_buy_cost_cents(self, order_id: str) -> int:
        with self._engine.begin() as conn:
            row = conn.execute(
                text("SELECT COALESCE(SUM(gross_notional_cents + fee_cents), 0) AS total FROM execution_fills WHERE order_id = :order_id"),
                {"order_id": order_id},
            ).first()
        return int(row.total if row else 0)

    def _mark_connection_failure(self, tenant_id: uuid.UUID, error_message: str) -> None:
        with self._engine.begin() as conn:
            row = conn.execute(
                text("SELECT COUNT(*) AS total FROM execution_reconciliation_events WHERE tenant_id = :tenant_id AND scope = 'connection_health' AND status = 'error'"),
                {"tenant_id": str(tenant_id)},
            ).first()
            ...
            conn.execute(
                ...
                {"status": status, "now": _utcnow(), "last_error": error_message[:512], "tenant_id": str(tenant_id)},
            )
            conn.execute(
                text(
                ...
                {"connected": connected, "status": status, "now": _utcnow(), "err": error_message[:512], "tenant_id": str(tenant_id)},
            )
        self._audit(
            tenant_id=tenant_id,
            trading_mode=TradingMode.LIVE,
        ...
        with self._engine.begin() as conn:
            conn.execute(
                ...
                ),
                {
                    ...
                    "status": status,
                    "detail": detail,
                    "internal_snapshot": json.dumps(internal_snapshot or {}, default=str),
                    "external_snapshot": json.dumps(external_snapshot or {}, default=str),
